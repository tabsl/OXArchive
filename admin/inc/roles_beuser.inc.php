<?php
/**
 *    This file is part of OXID eShop Community Edition.
 *
 *    OXID eShop Community Edition is free software: you can redistribute it and/or modify
 *    it under the terms of the GNU General Public License as published by
 *    the Free Software Foundation, either version 3 of the License, or
 *    (at your option) any later version.
 *
 *    OXID eShop Community Edition is distributed in the hope that it will be useful,
 *    but WITHOUT ANY WARRANTY; without even the implied warranty of
 *    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *    GNU General Public License for more details.
 *
 *    You should have received a copy of the GNU General Public License
 *    along with OXID eShop Community Edition.  If not, see <http://www.gnu.org/licenses/>.
 *
 * @link http://www.oxid-esales.com
 * @package inc
 * @copyright © OXID eSales AG 2003-2008
 * $Id: roles_beuser.inc.php 14035 2008-11-06 14:48:53Z arvydas $
 */

$aColumns = array( 'container1' => array(    // field , table,  visible, multilanguage, ident
                                        array( 'oxusername',  'oxuser', 1, 0, 0 ),
                                        array( 'oxlname',     'oxuser', 0, 0, 0 ),
                                        array( 'oxfname',     'oxuser', 0, 0, 0 ),
                                        array( 'oxstreet',    'oxuser', 0, 0, 0 ),
                                        array( 'oxstreetnr',  'oxuser', 0, 0, 0 ),
                                        array( 'oxcity',      'oxuser', 0, 0, 0 ),
                                        array( 'oxzip',       'oxuser', 0, 0, 0 ),
                                        array( 'oxfon',       'oxuser', 0, 0, 0 ),
                                        array( 'oxbirthdate', 'oxuser', 0, 0, 0 ),
                                        array( 'oxid',        'oxuser', 0, 0, 1 ),
                                        ),
                     'container2' => array(
                                        array( 'oxusername',  'oxuser', 1, 0, 0 ),
                                        array( 'oxlname',     'oxuser', 0, 0, 0 ),
                                        array( 'oxfname',     'oxuser', 0, 0, 0 ),
                                        array( 'oxstreet',    'oxuser', 0, 0, 0 ),
                                        array( 'oxstreetnr',  'oxuser', 0, 0, 0 ),
                                        array( 'oxcity',      'oxuser', 0, 0, 0 ),
                                        array( 'oxzip',       'oxuser', 0, 0, 0 ),
                                        array( 'oxfon',       'oxuser', 0, 0, 0 ),
                                        array( 'oxbirthdate', 'oxuser', 0, 0, 0 ),
                                        array( 'oxid',     'oxobject2role', 0, 0, 1 ),
                                        )
                    );
/**
 * Class manages back-end user rights
 */
class ajaxComponent extends ajaxListComponent
{
    /**
     * Returns SQL query for data to fetc
     *
     * @return string
     */
    protected function _getQuery()
    {
        $myConfig = $this->getConfig();

        // looking for table/view
        $sUserTable = getViewName('oxuser');

        $sRoleId      = oxConfig::getParameter( 'oxid' );
        $sSynchRoleId = oxConfig::getParameter( 'synchoxid' );

        // category selected or not ?
        if ( !$sRoleId) {
            // dodger performance
            $sQAdd  = " from $sUserTable where 1 ";
            if (!$myConfig->getConfigParam( 'blMallUsers' ) )
                $sSelect .= " and oxshopid = '".$myConfig->getShopId()."' ";
        } else {
            // selected group ?
            if ( $sSynchRoleId && $sSynchRoleId != $sRoleId ) {
                $sQAdd  = " from oxobject2group inner join $sUserTable on $sUserTable.oxid = oxobject2group.oxobjectid ";
                $sQAdd .= " where oxobject2group.oxgroupsid = '$sRoleId' ";
                if (!$myConfig->getConfigParam( 'blMallUsers' ) )
                    $sQAdd .= " and $sUserTable.oxshopid = '".$myConfig->getShopId()."' ";
            } else {
                $sQAdd  = " from oxobject2role, $sUserTable where oxobject2role.oxtype = 'oxuser' and ";
                $sQAdd .= " oxobject2role.oxroleid = '$sRoleId' and $sUserTable.oxid=oxobject2role.oxobjectid ";
            }
        }

        if ( $sSynchRoleId && $sSynchRoleId != $sRoleId) {
            // dodger performance
            $sQAdd .= " and $sUserTable.oxid not in ( select oxobject2role.oxobjectid  from oxobject2role where oxobject2role.oxtype = 'oxuser' and oxobject2role.oxroleid = '$sSynchRoleId' ) ";
        }

        return $sQAdd;
    }

    /**
     * Removes User from R&R
     *
     * @return null
     */
    public function removeuserfromberoles()
    {
        $aRemoveGroups = $this->_getActionIds( 'oxobject2role.oxid' );
        if ( oxConfig::getParameter( 'all' ) ) {

            $sQ = $this->_addFilter( "delete oxobject2role.* ".$this->_getQuery() );
            oxDb::getDb()->Execute( $sQ );

        } elseif ( $aRemoveGroups && is_array( $aRemoveGroups ) ) {
            $sQ = "delete from oxobject2role where oxobject2role.oxid in ('" . implode( "', '", $aRemoveGroups ) . "') ";
            oxDb::getDb()->Execute( $sQ );
        }
    }

    /**
     * Adds User to R&R
     *
     * @return null
     */
    public function addusertoberoles()
    {
        $aChosenUsr = $this->_getActionIds( 'oxuser.oxid' );
        $soxId      = oxConfig::getParameter( 'synchoxid' );

        if ( oxConfig::getParameter( 'all' ) ) {
            $sUserTable = getViewName('oxuser');
            $aChosenUsr = $this->_getAll( $this->_addFilter( "select $sUserTable.oxid ".$this->_getQuery() ) );
        }
        if ( $soxId && $soxId != "-1" && is_array( $aChosenUsr ) ) {
            foreach ( $aChosenUsr as $sChosenUsr) {
                $oRiRo = oxNew( "oxbase" );
                $oRiRo->init( "oxobject2role" );
                $oRiRo->oxobject2role__oxobjectid = new oxField($sChosenUsr);
                $oRiRo->oxobject2role__oxroleid   = new oxField($soxId);
                $oRiRo->oxobject2role__oxtype     = new oxField("oxuser");
                $oRiRo->save();
            }
        }
    }
}
