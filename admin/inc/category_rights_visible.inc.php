 ...
 * $Id: category_rights_visible.inc.php 14035 2008-11-06 14:48:53Z arvydas $
 */
...
class ajaxComponent extends ajaxListComponent
{
    ...
    protected function _getQuery()
    {
        ...
        $sCatId      = oxConfig::getParameter( 'oxid' );
        $sSynchCatId = oxConfig::getParameter( 'synchoxid' );
        ...
        if ( !$sCatId) {
            ...
            $sQAdd  .= " oxobjectrights.oxobjectid = '$sCatId' and ";
        ...
        if ( $sSynchCatId && $sSynchCatId != $sCatId) {
            $sQAdd   = " from $sGroupTable left join oxobjectrights on ";
            $sQAdd  .= " oxobjectrights.oxoffset = ($sGroupTable.oxrrid div 31) and ";
            $sQAdd  .= " oxobjectrights.oxgroupidx & (1 << ($sGroupTable.oxrrid mod 31)) and oxobjectrights.oxobjectid='$sSynchCatId' and oxobjectrights.oxaction = $iAction ";
            $sQAdd  .= " where oxobjectrights.oxobjectid != '$sSynchCatId' or (oxobjectid is null)";
        }
        ...
        if ( $soxId != "-1" && isset( $soxId) && is_array($aGroups) && count($aGroups) ) {
            ...
            foreach ( $aIndexes as $iOffset => $sIdx ) {
                ...
                if ($iRange) {
                    ...
                    $sQ .= "select oxobject2category.oxobjectid from $sO2CView oxobject2category ";
        ...
        if ( $soxId != "-1" && isset( $soxId) && is_array($aGroups) && count($aGroups)) {
            ...
            foreach ( $aIndexes as $iOffset => $sIdx ) {
                ...
                if ( $iRange ) {
                    ...
                    $sQ .= "from $sO2CView as oxobject2category ";
                    $sQ .= "left join oxcategories on oxcategories.oxid = oxobject2category.oxcatnid ";
