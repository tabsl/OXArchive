 ...
 * $Id: oxemail.php 13617 2008-10-24 09:38:46Z sarunas $
 */
/**
...
class oxEmail extends phpmailer
{
    ...
    public function sendOrderEmailToUser( $oOrder )
    {
        ...
        $this->setReplyTo( $oShop->oxshops__oxorderemail->value, $oShop->oxshops__oxname->value );
    ...
    public function sendForgotPwdEmail( $sEmailAddress )
    {
        ...
        $sSelect = "select oxid from oxuser where oxuser.oxactive = 1 and oxuser.oxusername = '$sEmailAddress' and oxuser.oxpassword != ''";
        ...
        if ( !empty($sOxId) ) {
            ...
            if ( $oUser->load($sOxId) ) {
                ...
                if ( strpos($sPassword, "ox_") === false )
                    ...
                    $oUser->oxuser__oxpassword = new oxField(oxUtils::getInstance()->strRem( $sPassword, $myConfig->getConfigParam( 'sConfigKey' ) ));
    ...
    public function sendNewsletterDbOptInMail( $oUser )
    {
        ...
        $this->setSubject( "Newsletter " . $oShop->oxshops__oxname->value );

        $sFullName = $oUser->oxuser__oxfname->value . " " . $oUser->oxuser__oxlname->value;

        $this->setRecipient( $oUser->oxuser__oxusername->value, $sFullName );
        $this->setFrom( $oShop->oxshops__oxinfoemail->value, $oShop->oxshops__oxname->value );
    ...
    public function sendStockReminder( $aBasketContents )
    {
        ...
        $this->setRecipient( $oShop->oxshops__oxowneremail->value, $oShop->oxshops__oxname->value );
        $this->setFrom( $oShop->oxshops__oxowneremail->value, $oShop->oxshops__oxname->value );
   ...
   */
    ...
    protected function _setMailParams( $oShop = null )
    {
        ...
        $this->setFrom( $oShop->oxshops__oxorderemail->value, $oShop->oxshops__oxname->value );
        $this->setSmtp( $oShop );
    }
