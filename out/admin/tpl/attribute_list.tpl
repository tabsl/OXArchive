...
[{foreach from=$mylist item=listitem}]
    ...
    [{ /if}]
    <td valign="top" class="[{ $listclass}]" height="15"><div class="listitemfloating">&nbsp;<a href="Javascript:EditThis('[{ $listitem->oxattribute__oxid->value}]');" class="[{ $listclass}]">[{ if $listitem->oxattribute__oxtitle->value }][{ $listitem->oxattribute__oxtitle->value }][{else}]--[{/if}]</a></div></td>
    <td class="[{ $listclass}]">
      [{if !$readonly }]
