    ...
    [{foreach from=$menustructure item=menuholder }]
        [{if $menuholder->nodeType == XML_ELEMENT_NODE && $menuholder->childNodes->length }]
            ...
            [{foreach from=$menuholder->childNodes item=menuitem }]
                ...
                <optgroup label="[{ oxmultilang ident=$menuitem->getAttribute('name')|default:$menuitem->getAttribute('id') }]">
                    [{foreach from=$menuitem->childNodes item=submenuitem }]
                        [{if $submenuitem->nodeType == XML_ELEMENT_NODE}]
                            <option value="[{ $submenuitem->getAttribute('cl') }]" [{if in_array($submenuitem->getAttribute('cl'),$aFavorites) }]selected[{/if}]>[{ oxmultilang ident=$submenuitem->getAttribute('name')|default:$submenuitem->getAttribute('id') }]</option>
                        [{/if}]
                    [{/foreach}]
                </optgroup>
                [{/if}]
