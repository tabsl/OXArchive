...
<tr class="listitem">
    <td valign="top" class="listfilter first" height="20" colspan="2">
        ...
        <input class="listedit" type="text" size="50" maxlength="128" name="where[[{$listTable}].oxtitle]" value="[{ $where->oxdiscount__oxtitle }]">
        </div></div>
    </td>
</tr>
<tr>
    <td class="listheader first" height="15" colspan="2">&nbsp;<a href="Javascript:document.search.sort.value='oxtitle';document.search.submit();" class="listheader">[{ oxmultilang ident="GENERAL_DESCRIPTION" }]</a></td>
</tr>
...
[{foreach from=$mylist item=listitem}]
    ...
    [{ /if}]
    <td valign="top" class="[{ $listclass}]" height="15"><div class="listitemfloating">&nbsp;<a href="Javascript:EditThis('[{ $listitem->oxdiscount__oxid->value}]');" class="[{ $listclass}]">[{ $listitem->oxdiscount__oxtitle->value }]</a></div></td>
    <td class="[{ $listclass}]">
    [{if !$readonly }]
        <a href="Javascript:DeleteThis('[{ $listitem->oxdiscount__oxid->value }]');" class="delete" id="del.[{$_cnt}]" title="" [{include file="help.tpl" helpid=item_delete}]></a>
    [{/if}]
    </td>
</form></tr>
[{if $blWhite == "2"}]
