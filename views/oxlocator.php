 ...
 * $Id: oxlocator.php 13629 2008-10-24 11:33:47Z vilma $
 */
...
class oxLocator extends oxSuperCfg
{
     ...
     * @param oxubase   $oLocatorTarget oxview object
     * @param string    $sUrl           current or user defined url
     *
     * @return null
     */
    public function setLocatorData( $oCurrArticle, $oLocatorTarget )
    {
        ...
        $oLocatorTarget->setListType( 'sListType', $this->_sType );
    }
     ...
     * <b>actCatpath</b>
     *
     * @param object    $oLocatorTarget oxview object
     * @param oxarticle $oCurrArticle   current article
     *
     * @return null
     */
    protected function _setVendorLocatorData( $oLocatorTarget, $oCurrArticle )
    {
        if ( ( $oVendor = $oLocatorTarget->getActVendor() ) ) {
            ...
            if ( $this->_oNextProduct ) {
                $this->_oNextProduct->setLinkType( 1 );
            }

            if ( $this->_oBackProduct ) {
                $this->_oBackProduct->setLinkType( 1 );
            }
     ...
     * <b>searchparamforhtml</b>, <b>searchcnid</b>, <b>searchvendor</b>
     *
     * @param object    $oLocatorTarget oxview object
     * @param oxarticle $oCurrArticle   current article
     *
    ...
    protected function _setSearchLocatorData( $oLocatorTarget, $oCurrArticle )
    {
        if ( ( $oSearchCat = $oLocatorTarget->getActSearch() ) ) {
            ...
            $oIdList->loadSearchIds( $sSearchParam, $sSearchCat, $sSearchVendor );
    ...
    protected function _setTagLocatorData( $oLocatorTarget, $oCurrArticle )
    {
        if ( ( $oTag = $oLocatorTarget->getActTag() ) ) {
            ...
            $oIdList->getTagArticleIds( $oTag->sTag, oxLang::getInstance()->getBaseLanguage() );
            ...
            $oLocatorTarget->setSearchTitle( oxLang::getInstance()->translateString('TAGS').' / '.htmlspecialchars( ucfirst( $oTag->sTag ) ) );
            // for compatibility reasons for a while. will be removed in future
            $oLocatorTarget->addTplParam( 'sSearchTitle', $oLocatorTarget->getSearchTitle() );
    ...
    protected function _setRecommlistLocatorData( $oLocatorTarget, $oCurrArticle )
    {
        if ( ( $oRecommList = $oLocatorTarget->getActRecommList() ) ) {
            ...
            if ( $sSearchRecomm !== null ) {
                $sTitle .= " / ".oxLang::getInstance()->translateString('RECOMMLIST_SEARCH').' "'.$sSearchFormRecomm.'"';
